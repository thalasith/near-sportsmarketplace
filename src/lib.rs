...
impl NBABetsDate {
    ...
    #[payable]
    pub fn set_bet(&mut self, better_amount:U128, game_id: String, game_date: String, market_maker_team: String, bidder_team: String, start_time_utc: String) {
        let game_day = game_date.clone();
        let game_id = game_id.clone();
        // let game_data = request_game(&game_date, &game_id);
    ...
    #[payable]
    pub fn accept_bet_index(&mut self, index: usize) {
        let mut bet = self.get_bet_by_index(index);
    ...
    #[private]
    fn return_funds_to_maker(&mut self, index:usize) -> Promise {
        let bet = self.get_bet_by_index(index);
        Promise::new(bet.market_maker_id).transfer(bet.market_maker_deposit.0)
    }

    #[private]
    fn return_funds_to_better(&mut self, index:usize) -> Promise {
        let bet = self.get_bet_by_index(index);
        assert!(bet.better_found == true, "No better was found.");
        Promise::new(bet.better.unwrap()).transfer(bet.better_deposit.0)
    }

    pub fn cancel_bet(&mut self, index:usize) {
        let bet = self.get_bet_by_index(index);
        ...
        if bet.better_found == true  { 
            ...
            self.return_funds_to_better(index);
        };
        self.return_funds_to_maker(index);
        let mut bets_by_day = self.get_all_bets();
    ...
    pub fn payout_bet(&mut self, index: usize, winning_team: String) {
        let mut bet = self.get_bet_by_index(index);
        assert!(bet.paid_out == false, "This bet as already been paid out.");
    ...
    // Lock contract
    pub fn lock_contract(&mut self, index: usize) {
        let mut bet = self.get_bet_by_index(index);
    ...
    pub fn get_bet_by_index(&self, index: usize ) -> NBABet{
        self.get_all_bets()[index].clone()
    }
}
...
mod tests {
    ...
    fn test_get_all_bets() {
        ...
        contract.set_bet(ntoy(5).into(),"0022100488".to_string(), "20211225".to_string(), "ATL".to_string(), "NYK".to_string(), "2021-12-25T17:00:00.000Z".to_string());
        // Boston (1610612738) vs Milwaukee (1610612749)
        contract.set_bet(ntoy(10).into(),"0022100489".to_string(), "20211225".to_string(), "BOS".to_string(), "MIL".to_string(), "2021-12-25T17:30:00.000Z".to_string());
    ...
    fn get_bet_panic() {
        ...
        contract.get_bet_by_index(1);
    }

    #[test]
    fn accept_bet_by_index() {
        let mut context = get_context();
        context.attached_deposit = ntoy(10).into();
        testing_env!(context.clone());
        // let mut contract = NBABet::new(AccountId::try_from(context.current_account_id.clone()).unwrap());
        ...
        assert_eq!(contract.get_bet_by_index(0).better_found, true);
        assert_eq!(contract.get_bet_by_index(0).better.unwrap().as_str(), "boris.testnet");
    }
